...
from ClassifyUsingSimilarity import classifyBySimilarity
...
class Model:
    ...
    def outputClusteringResult(self, outputDir, documentSet):
        ...
        globalOutPath="result/NewsPredTueTextMStream.txt"
        f = open(globalOutPath, 'a')
		
        print("start doc="+str(self.startDoc)+", end doc="+str(self.currentDoc))			
        for d in range(self.startDoc, self.currentDoc):           	
            documentID = documentSet.documents[d].documentID			
            cluster = self.z[documentID]
            batchDocs.append(documentSet.documents[d]) #rakib				
            #rakib  			
            #print(str(documentID)+","+str(cluster)+","+documentSet.documents[d].text) 
            #self.docsPerCluster[cluster]
            #if cluster not in self.docsPerCluster:
            #  self.docsPerCluster[cluster] = [documentSet.documents[d]]
            #else:
            #  self.docsPerCluster[cluster] = [self.docsPerCluster[cluster], documentSet.documents[d]] #.extend(documentSet.documents[d])
            
			#if cluster not in self.docsPerCluster:
            # print("new cluster="+str(cluster))
            #else:
            # print("train the classification model and ") 
            #
            #print("cluster="+str(cluster)+", #docs="+ str(len(self.docsPerCluster[cluster])))			
        ...
        for d in range(self.startDoc, self.currentDoc):            		
            documentID = documentSet.documents[d].documentID			
            cluster = self.z[documentID]
            self.docsPerCluster.setdefault(cluster,[]).append(documentSet.documents[d]) #rakib			
			
        #for key, value in self.docsPerCluster.items():
        #    print("rakib at each iteration, cluster id="+str(key)+", items="+str(len(value)))
            #wordSet=set()
        #    mergeDic = Counter()			
        #    for document in value:
        #        docTxtArr = document.text.split()
        #        print(document.text)
                #wordSet.update(docTxtArr)
        #        mergeDic= mergeDic + Counter(docTxtArr)
            #print(wordSet)				
        #    print(mergeDic)		     
        
        ##############rakib semantic classification##################
		    #self.docsPerCluster.setdefault 
		    #do some adjustment
        docsPerClusterToBeUsedForTraining = {} #not using
        wordsPerClusterToBeUsedForTraining = {}
        docsPerClusterToBeReclassified = {}		
        for key, value in self.docsPerCluster.items():
            print("rakib at each iteration, cluster id="+str(key)+", items="+str(len(value)))		
            clusterid = key
            docs = value
            if len(docs)==1:
                print("single cluster="+str(clusterid)+","+docs[0].text)
                docsPerClusterToBeReclassified.setdefault(clusterid,[]).append(docs[0])  				
            if len(docs)>1:                
                mergeDic = Counter()			
                ...
                mergeDic=Counter(el for el in mergeDic.elements() if mergeDic[el] >= 2)				                   
                if len(mergeDic)==0:
                    for doc in docs:
        ...
        print("#docsPerClusterToBeReclassified="+str(len(docsPerClusterToBeReclassified)))
        print("#docsPerClusterToBeUsedForTraining="+str(len(docsPerClusterToBeUsedForTraining)))
        print("rakib #batchDocs="+str(len(batchDocs)))	
        ...
        docsVectorPerClusterToBeReclassified = {}
        ...
        for clusterid, docs in docsPerClusterToBeReclassified.items():
            print("clusterid="+str(clusterid)+",docs="+str(len(docs)))
            for doc in docs:
               wordArr = doc.text.strip().split()
               print("wordArr to be classified="+str(wordArr))
               ...
               #print("singleDocVector="+str(singleDocVector))
               docsVectorPerClusterToBeReclassified.setdefault(clusterid,[]).append(singleDocVector)

        #call similarity based classification by 
        docsLabelPerClusterForToBeReclassified=classifyBySimilarity(wordsVectorPerClusterForTraining, docsVectorPerClusterToBeReclassified, docsPerClusterToBeReclassified)         
        #########end###############	
        globalOutPathSemantic="result/NewsPredTueTextMStreamSemantic.txt"		
        fSemantic = open(globalOutPathSemantic, 'a')		
        for clusterid, docs in docsPerClusterToBeUsedForTraining.items():
            for doc in docs:
                fSemantic.write(str(clusterid)+"	"+str(doc.clusterNo)+"	"+doc.text+"\n") 		
        
        for clusterid, docs in docsLabelPerClusterForToBeReclassified.items():
            for doc in docs:
                fSemantic.write(str(clusterid)+"	"+str(doc.clusterNo)+"	"+doc.text+"\n") 		
