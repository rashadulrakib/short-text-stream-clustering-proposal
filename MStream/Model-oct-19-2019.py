...
class Model:
    ...
    def detectOutlierAndEnhanceByEmbeddingSimProduct(self, documentSet, wordVectorsDic):
      batchDocs, pred_true_text_inds, maxPredLabel=self.populateBatchDocs(documentSet)
      ...
      appendResultFile(cleaned_non_outlier_pred_true_txt_ind_prevPreds, 'result/mstr-enh')
      ...
      for label, cluster_pred_true_txt_inds in dic_itemGroups.items():
        ...
        #self.customGibbsSampling(outlier_pred_true_text_ind_prevPreds, documentSet, wordVectorsDic)		
      ...
      for i in range(self.iterNum):
        ...
        for pred_true_text_ind_prevPred in pred_true_text_ind_prevPreds:
          ...
          if i != self.iterNum - 1:  # if not last iteration
            cluster = self.customSampleCluster(d, document, documentID, 0, wordVectorsDic)
          elif i == self.iterNum - 1:  # if last iteration
            cluster = self.customSampleCluster(d, document, documentID, 1, wordVectorsDic)
