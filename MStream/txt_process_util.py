...
from compute_util import ComputeEntropy
from compute_util import computeTextSimCommonWord
from compute_util import compute_sim_value
#from compute_util import computeTextSimCommonWord_WordArr
...
def createBinaryWordCooccurenceMatrix(listtuple_pred_true_text):
  ...
  for i in range(len(listtuple_pred_true_text)):
    ...
    #word_tokenize(listtuple_pred_true_text[i][2])
    #print(words)	
...
def RemoveHighClusterEntropyWordsIndex(pred_true_txt_ind_prevPreds):
  ...
  dic_txt_to_cluster, dic_word_to_txt=getDicTxtToClass_WordToTxt(pred_true_txt_ind_prevPreds) #dic_txt_to_cluster (txt_0-> cluster1)
  dic_word_to_clusterEntropy, dic_word_to_cluster_indecies=getDicWordToClusterEntropy(dic_txt_to_cluster,dic_word_to_txt)
  ...
  for i in range(len(pred_true_txt_ind_prevPreds)):
    ...
    cleaned_word_arr = removeWordsByHighEntropy(word_arr, dic_word_to_clusterEntropy, dic_word_to_cluster_indecies,high_entropy_words)
...
def semanticSims(text_Vec, dic_centerVecs, dic_cluster_noTexts):
  dic_semanticSims={}
  ...
  for label, centVec in dic_centerVecs.items():
    str_label=str(label)
    cluster_noTexts=0	
    if str_label in dic_cluster_noTexts.keys():    	
      cluster_noTexts=dic_cluster_noTexts[str_label]
    elif label in dic_cluster_noTexts.keys():
      cluster_noTexts=dic_cluster_noTexts[label]	
